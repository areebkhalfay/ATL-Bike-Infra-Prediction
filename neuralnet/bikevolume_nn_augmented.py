...
class BikeDataset(Dataset):
    def __init__(self, features, targets, device):
        ...
        self.targets = torch.tensor(targets, dtype=torch.float32).reshape(-1, 1).to(device)
...
def prepare_data(file_path, device, batch_size=64, use_log=True, pca_variance_threshold=0.95, augmentation_factor=2, noise_level=0.05):
    """
    Load the NYC data from a csv file, apply stratified sampling for train/test split,
    perform data augmentation by adding noise, apply PCA to retain components explaining 
    a specified proportion of variance, save PCA parameters, and pass it into a dataloader for training
    ...
    - augmentation_factor: How many times to augment each data point (default: 2)
    - noise_level: Standard deviation of Gaussian noise for augmentation (default: 0.05)
    ...
    # Create bins for stratified sampling based on target values
    # This helps ensure balanced distribution of target values in train and test sets
    num_bins = 10  # Can be adjusted based on dataset distribution
    y_binned = pd.qcut(y, num_bins, labels=False, duplicates='drop')
    
    # Split data before applying PCA to avoid data leakage, using stratified sampling
    ...
    # Perform data augmentation on the training set in PCA space
    if augmentation_factor > 0:
        X_train_augmented = [X_train_pca]
        y_train_augmented = [y_train.values]
        
        for i in range(augmentation_factor):
            # Generate random noise with a controlled magnitude
            noise = np.random.normal(0, noise_level, X_train_pca.shape)
            X_augmented = X_train_pca + noise
            
            # Also add a small amount of noise to target values
            y_noise = np.random.normal(0, noise_level * 0.1, y_train.shape)
            y_augmented = y_train.values + y_noise
            
            X_train_augmented.append(X_augmented)
            y_train_augmented.append(y_augmented)
        
        # Combine original and augmented data
        X_train_pca = np.vstack(X_train_augmented)
        y_train_values = np.concatenate(y_train_augmented)
        
        print(f"Data augmentation complete. Training set expanded from {len(y_train)} to {len(y_train_values)} samples")
    else:
        y_train_values = y_train.values
    
    # Create datasets
    train_dataset = BikeDataset(X_train_pca, y_train_values, device)
    test_dataset = BikeDataset(X_test_pca, y_test.values, device)
...
def train_model(model, train_loader, test_loader, num_epochs=100, learning_rate=0.001, weight_decay=0.001, patience=5000, window_size=20, pca_components=None, batch_size=None, dropout=None, device = 'cpu'):
    """
...
def custom_bike_loss(outputs, targets, alpha=0.7, beta=0.3):
    """
    Custom loss that combines MSE with R²-oriented loss component
    
    Parameters:
    - alpha: Weight for MSE component
    - beta: Weight for R² component
    """
    ...
    # R²-oriented component
    mean_target = torch.mean(targets)
    ss_tot = torch.sum((targets - mean_target) ** 2)
    ss_res = torch.sum((targets - outputs) ** 2)
    r2_component = ss_res / (ss_tot + 1e-8)  # Add small epsilon to avoid division by zero
    
    # Combined loss (lower is better)
    combined_loss = alpha * mse_loss + beta * r2_component
